# Starter: spaceport bar exercise (Python)
...
files in and restart. `backend/app/loader.py` reads them at startup into
`app.state.recipes`. Each record is a dict with:
...
- `backend/app/matching.py` — search/matching logic (empty, TODO)
- `backend/app/llm.py` — LLM integration (empty, TODO)
- `backend/app/main.py` — `POST /recommendations` currently returns `[]`
